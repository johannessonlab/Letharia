...
### Make trees between between Letharia species and homeologs within the L. lupina triploid
#############################################################################
...
# vcfdf_gtf$ALT <- factor(vcfdf_gtf$ALT, levels = vcfdf_gtf$ALT %>% unique %>% rev) # %>% droplevels) #NEW
...
# Filter the data to retain only the contig containing the MAT locus
genotypeguess_all_nomiss_mat <- genotypeguess_all_nomiss %>% filter(CHROM == matctg)
...
window_data <- winSNPsperwin(genotypeguess_all_nomiss %>% filter(CHROM == matctg), workingdir)
